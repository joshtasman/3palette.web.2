(function () {
  ...
  function suggestPalette() {
    ...
    var rolesToFill = pickTargetRoles(existingRoles, emptyIndices.length);

    // Pick order: best anchoring first. Starting from a blank palette, that means the
    // accent (from curated hues) before either neutral, since neutrals coordinate far
    // better once there's a real hue to work from than a random one would give them.
    ...
    // Display order: light to bold, independent of the pick order above.
    ...
    emptyIndices.forEach(function (idx, n) {
    ...
    updateSaveState();
    updateSuggestState();
    showToast("Here's a palette to get you started.");
  }
